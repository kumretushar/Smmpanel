...
CHANNEL_ID = "@gmtusharxfiles"  # Channel username
...
# ---------- CACHE ----------
user_cache = {}
force_join_cache = {}  # ✅ New cache for force-join status
...
def clear_cache():
    user_cache.clear()
    force_join_cache.clear()  # ✅ Clear force-join cache too
...
async def check_force_join(user_id):
    ...
    """
    # Check cache first
    if user_id in force_join_cache:
        return force_join_cache[user_id]
    
    try:
        member = await app.get_chat_member(CHANNEL_ID, user_id)
        is_member = member.status in ["member", "administrator", "creator"]
        force_join_cache[user_id] = is_member
        return is_member
    except Exception as e:
        print(f"Force join check error for {user_id}: {e}")
        force_join_cache[user_id] = False
        return False

async def refresh_force_join(user_id):
    """
    Force refresh the force-join check (bypass cache).
    Called when user clicks "I Joined" button.
    """
    try:
        member = await app.get_chat_member(CHANNEL_ID, user_id)
        is_member = member.status in ["member", "administrator", "creator"]
...
async def start(client, message):
    ...
    # ✅ Check force join (uses cache)
    is_member = await check_force_join(user_id)
...
async def check_join_callback(client, callback):
    ...
    # ✅ Force refresh the check
    is_member = await refresh_force_join(user_id)
    ...
    if is_member:
        ...
        await callback.message.edit_text(
            ...
            f"🎉 Thanks for joining! Use the buttons below."
        )
        ...
        await callback.answer("✅ Joined successfully!", show_alert=True)
    else:
        await callback.answer(
            "❌ You haven't joined yet. Please join the channel and click again.",
            show_alert=True
        )
...
async def get_number(client, message):
    ...
    if not await check_force_join(user_id):
        await message.reply(
            f"⚠️ Join channel first: {CHANNEL_ID}",
            reply_markup=InlineKeyboardMarkup([
...
async def traffic_stats(client, message):
    ...
    if not await check_force_join(user_id):
        await message.reply(f"⚠️ Join channel first: {CHANNEL_ID}")
        return
