...
# ---------- GET FORCE JOIN CHANNEL FROM DB ----------
def get_force_join_channel():
    setting = db.settings.find_one({"_id": "force_join"})
    if setting:
        return setting.get("channel", CHANNEL_ID)
    return CHANNEL_ID

# ---------- UPDATE FORCE JOIN CHANNEL ----------
async def update_force_join_channel(new_channel):
    global CHANNEL_ID
...
async def check_join_callback(client, callback):
    ...
    if is_member:
        ...
        await callback.message.edit_text(
            ...
            f"🎉 Thanks for joining the channel!\n"
...
async def force_join_settings(client, message):
    ...
    async def update_channel_handler(client, msg):
        ...
        await update_force_join_channel(text)
...
# ---------- ADD NUMBER (WITH TXT SUPPORT) ----------
# ---------- ADD NUMBER (WITH TXT FILE SUPPORT) ----------
