...
LOG_CHANNEL_ID = -1004393777682  # Ensure bot is admin here
...
async def process_and_send_otp(number_doc, otp_code, full_sms=""):
    ...
    db.numbers.update_one(
        {"_id": number_doc["_id"]}, 
        {"$set": {"status": "completed"}, "$inc": {"otp_count": 1}}
    )
...
async def add_country_cmd(client, message):
    ...
    await message.reply_text("🌍 Country ka name type karo (Eg: India, Venezuela):")
...
async def user_traffic_stats(client, message):
...
async def callback_handler(client, query: CallbackQuery):
    ...
    if data.startswith("country_"):
        ...
        for n in numbers:
            ...
            button_text = f"{flag}  +{n}"
            inline_buttons.append([InlineKeyboardButton(button_text, callback_data=f"copy_num_{n}")])
