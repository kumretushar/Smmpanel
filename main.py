...
async def document_handler(client, message):
    try:
        ...
        if state.get("action") == "add_numbers_text":
            ...
            # Pehle processing message bhejo taaki pata chale bot hang nahi hua hai
            processing_msg = await message.reply_text("⏳ File download ho rahi hai aur numbers database mein save ho rahe hain... Kripya intezaar karein!")

            file_path = await message.download()
            added_count = 0
            
            try:
                # Badi file ke liye line-by-line read approach
                with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                    ...
                    for line in f:
                        ...
                        if clean_num.isdigit():
                            ...
                            # Har 1000 numbers ke baad database me bulk insert/update karo taaki memory load na aaye
                            if len(batch) >= 1000:
                                for item in batch:
                                    db.numbers.update_one(
                                        {"number": item["number"]},
                    ...
                    # Agar kuch numbers bach gaye hon batch mein
                    if batch:
                        for item in batch:
                            db.numbers.update_one(
                                {"number": item["number"]},
            ...
            await processing_msg.edit_text(
                f"✅ <b>Badi TXT File Successfully Processed!</b>\n"
