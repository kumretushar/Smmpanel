...
async def document_handler(client, message):
    try:
        ...
        if state.get("action") == "add_numbers_text":
            ...
            processing_msg = await message.reply_text("⏳ Fast speed se file process ho rahi hai, kripya thoda intezaar karein...")
                ...
                with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                    for line in f:
                        ...
                        if clean_num.isdigit():
                            ...
                            # Har 5000 numbers ka ek bulk batch banakar database ko bhej do
                            if len(bulk_operations) >= 5000:
                                db.numbers.bulk_write(bulk_operations, ordered=False)
                                bulk_operations = []
                
                # Agar bache hue numbers hain toh unhe bhi bhej do
                if bulk_operations:
                    db.numbers.bulk_write(bulk_operations, ordered=False)
                    
            except Exception as e:
                print(f"Bulk File read error: {e}")
