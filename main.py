...
def get_flag_from_country_name(country_name):
    """Converts full country name into flag unicode."""
    try:
...
def get_flag_from_number(number):
    """Detects Country Code from phone number & Returns Flag Unicode."""
...
async def allocate_three_numbers(user_id, service, country_name=None):
    try:
        ...
        if not number_docs and country_name and country_name != "ALL":
            query.pop("country")
            number_docs = list(db.numbers.find(query).limit(3))
...
async def callback_handler(client, query: CallbackQuery):
    try:
        ...
        if data == "check_join_sub":
            ...
            if not is_admin(user_id): return
            c_name = data.replace("del_country_", "")
            ...
            numbers, error = await allocate_three_numbers(user_id, service, country_name=country_name if country_name != "ALL" else None)
            if error:
            ...
            parts = data.split("_", 2)
            service = parts[1]
            country_name = parts[2] if len(parts) > 2 else "ALL"
                            ...
                            db.numbers.update_one(
                                {"number": clean_num},
                                {
                                    "$set": {
                                        ...
                                        "assigned_to": None,
                                        "service": service,
                                        "country": country_name if country_name != "ALL" else "ALL",
                                        "otp_count": 0
                                    }
                                },
                                upsert=True
...
async def central_state_handler(client, message):
    try:
        ...
        if action == "add_service_name" and is_admin(user_id):
            ...
            if row:
                inline_kb.append(row)
            inline_kb.append([InlineKeyboardButton("🌐 ANY / Auto Detect", callback_data=f"addnum_cc_{service_name}_ALL")])
            ...
            inline_kb.append([InlineKeyboardButton("🌐 Any / Random", callback_data=f"country_{text}_ALL")])
