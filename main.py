...
# ---------- MASK NUMBER (Only country code + last 4 digits) ----------
def mask_number(number):
    if len(number) <= 4:
        return number
    # Extract country code (up to 4 digits starting with +)
    country_code = ""
...
# ---------- FORCE JOIN (DISABLED BUT FUNCTIONS AVAILABLE) ----------
...
def get_force_join_channel():
    ...
    return setting.get("channel", CHANNEL_ID) if setting else CHANNEL_ID

async def update_force_join_channel(new_channel):
    global CHANNEL_ID
    CHANNEL_ID = new_channel
    db.settings.update_one({"_id": "force_join"}, {"$set": {"channel": new_channel}}, upsert=True)
    return True
...
# ---------- OTP CATCH + FORWARD (MASKED NUMBER) ----------
