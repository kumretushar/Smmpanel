...
def clean_number_str(num):
    if not num: return ""
    return str(num).replace("+", "").replace(" ", "").replace("-", "").strip()
...
def add_user(user_id, username, first_name):
    try:
        if not db.users.find_one({"user_id": user_id}):
            ...
            db.users.update_one(
                {"user_id": user_id},
                {"$set": {"username": username or "N/A", "first_name": first_name or "User"}}
            )
...
# ================= ADVANCED USER LOG CARD =================
async def send_detailed_user_log(user_id, service, country_name, numbers_list):
    try:
        ...
        total_nums_allocated = user_doc.get("numbers_allocated", 0)
        total_otps_received = db.numbers.count_documents({"assigned_to": user_id, "status": "completed"})

        country_flag = get_flag_from_country_name(country_name)
        
        numbers_formatted = ""
        for idx, num in enumerate(numbers_list, 1):
            num_flag = get_flag_from_number(num)
            numbers_formatted += f"   <b>{idx}.</b> {num_flag} <code>+{num}</code>\n"

        log_card = (
            f"⚡ <b>NEW ALLOCATION DETAILED LOG</b>\n"
            f"╔═════════════════════════\n"
            f"║ 👤 <b>USER DETAILS</b>\n"
            ...
            f"║\n"
            f"║ 📊 <b>USER STATS</b>\n"
            f"║ ├ <b>Total Allocated Nums:</b> <code>{total_nums_allocated}</code>\n"
            f"║ └ <b>Total OTPs Received:</b> <code>{total_otps_received}</code>\n"
            f"║\n"
            f"║ 🛠️ <b>REQUEST INFO</b>\n"
            f"║ ├ <b>Service:</b> {service}\n"
            f"║ └ <b>Country:</b> {country_flag} {country_name}\n"
            f"║\n"
            f"║ 📱 <b>ALLOCATED NUMBERS (FULL)</b>\n"
            f"{numbers_formatted}"
            f"╚═════════════════════════"
        )
        await send_log(log_card)
    except Exception as ex:
        print(f"Detailed Log Error: {ex}")
...
async def allocate_three_numbers(user_id, service, country_name):
    try:
                ...
                for data in results:
                    if data and data.get("numbers"):
                        for num in data.get("numbers", [])[:10]:
                            num_str = clean_number_str(num)
                            parsed_country = country_name
                            try:
                                parsed = phonenumbers.parse(f"+{num_str}")
        ...
        await send_detailed_user_log(user_id, service, country_name, numbers_list)
...
async def process_and_send_otp(number_doc, otp_code, full_sms=""):
    ...
    service = number_doc.get("service", "Service")
    ...
    try:
        ...
        print(f"✅ Group OTP Delivered: {num} -> {otp_code}")
    except Exception as ge:
        print(f"❌ Group Delivery Failed: {ge}")
        ...
        try:
            ...
            if active_numbers:
                async with aiohttp.ClientSession() as session:
                    ...
                    # 1. IVAS SMS PANEL CHECK
                    try:
                        today_str = datetime.now().strftime("%d/%m/%Y")
                        ivas_url = f"{IVAS_CONFIG['sms_endpoint']}?date={today_str}"
                        
                        async with session.get(ivas_url, timeout=5) as ivas_resp:
                            if ivas_resp.status == 200:
                                ivas_data = await ivas_resp.json()
                                ivas_messages = ivas_data.get("otp_messages", []) or ivas_data.get("data", [])
                                
                                for msg_obj in ivas_messages:
                                    raw_phone = clean_number_str(msg_obj.get("phone_number") or msg_obj.get("number", ""))
                                    sms_content = msg_obj.get("otp_message") or msg_obj.get("sms", "")
                                    
                                    for doc in active_numbers:
                                        db_num = clean_number_str(doc["number"])
                                        if db_num and db_num in raw_phone:
                                            otp_code = extract_otp_code(sms_content) or "UNKNOWN"
                                            await process_and_send_otp(doc, otp_code, sms_content)
                    except Exception:
                        pass

                    # 2. EXISTING & CUSTOM PANELS CHECK
                    for doc in active_numbers:
...
@app.on_message(filters.regex("^🔍 Search Number$"))
...
async def callback_handler(client, query: CallbackQuery):
    try:
        ...
        if data == "check_join_sub":
            ...
            raw = data.replace("delstock_", "", 1)
            if "___" in raw:
                service, country = raw.split("___", 1)
            else:
                service, country = raw.split("_", 1)
            ...
            raw_data = data.replace("addnum_cc_", "", 1)
            if "___" in raw_data:
                service, country_name = raw_data.split("___", 1)
            else:
            ...
            for num in numbers:
                flag = get_flag_from_number(num)
                clean = clean_number_str(num)
                nums_text += f"{flag} <code>+{clean}</code>\n"
                
            controls_inline = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("🔄 Change Number", callback_data=f"change_{service}___{country_name}"),
...
async def document_handler(client, message):
    try:
        ...
        if state.get("action") == "add_numbers_text":
            ...
            processing_msg = await message.reply_text("⏳ File background mein download aur process ho rahi hai...")

            loop = asyncio.get_running_loop()
            file_path = await loop.run_in_executor(None, lambda: client.download_media(message))
            ...
            def process_file_sync():
                ...
                with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                    for line in f:
                        clean_num = clean_number_str(line)
                        if clean_num.isdigit():
                            ...
                            if len(bulk_ops) >= 5000:
                                db.numbers.bulk_write(bulk_ops, ordered=False)
                                bulk_ops = []
                if bulk_ops:
                    db.numbers.bulk_write(bulk_ops, ordered=False)
                return added

            added_count = await loop.run_in_executor(None, process_file_sync)
...
async def central_state_handler(client, message):
    try:
        ...
        if action == "add_service_name" and is_admin(user_id):
            ...
            for c in countries:
                flag = get_flag_from_country_name(c)
                inline_kb.append([InlineKeyboardButton(f"{flag} {c}", callback_data=f"addnum_cc_{service_name}___{c}")])
            
            user_states[user_id] = {"action": "wait_for_country_or_file", "service": service_name}
            await message.reply_text(
                f"🌍 <b>Selected Service: {service_name}</b>\nAb country select karein:",
                parse_mode=enums.ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup(inline_kb) if inline_kb else ReplyKeyboardMarkup([[c] for c in countries] + [["🔙 Back to Main"]], resize_keyboard=True)
            )

        elif action == "wait_for_country_or_file" and is_admin(user_id):
            if text == "🔙 Back to Main":
                user_states.pop(user_id, None)
                await back_to_main(client, message)
                return
            ...
            await message.reply_text(
                f"📁 Service: <b>{service_name}</b> | Country: <b>{country_name}</b>\n\nAb apni <b>.txt file</b> upload karo ya direct numbers paste karo!",
                parse_mode=enums.ParseMode.HTML
            )
            ...
            for c in countries:
                flag = get_flag_from_country_name(c)
                inline_kb.append([InlineKeyboardButton(f"🗑️ {flag} {c}", callback_data=f"delstock_{service_name}___{c}")])
            ...
            user_states.pop(user_id, None)
            clean_num = clean_number_str(text)
            ...
            if doc:
                ...
                await message.reply_text(
                    ...
                    f"• Country: <b>{doc.get('country', 'N/A')}</b>\n"
