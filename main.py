...
# ---------- CACHE & STATES ----------
user_cache = {}
user_states = {}  # Global state manager to fix dynamic handler bugs
...
async def manage_services(client, message):
    ...
    services_list = [s.get("name") for s in services_from_db if s.get("name") and s.get("name") != "SYSTEM"]
...
async def central_state_handler(client, message):
    ...
    # ----- Admin State Actions -----
        ...
        await message.reply(f"✅ {len(numbers)} numbers detected.\nSend service name (e.g. WhatsApp):")
        ...
        for num in numbers:
            if num and not db.numbers.find_one({"number": num}):
                db.numbers.insert_one({"number": num, "country": c_name, "country_code": code, "flag": flag, "service": service, "status": "available", "assigned_to": None, "otp_received": [], "otp_count": 0})
                added += 1
        user_states.pop(user_id, None)
        await message.reply(f"✅ Added {added} numbers to '{service}'.")
        ...
        await message.reply(f"✅ Service '{text}' created!")
        ...
        await message.reply(f"✅ Service '{text}' removed ({cnt} numbers deleted).")
